// ═══════════════════════════════════════════════════════════
//  LAS 7 CACAS MAESTRAS — Sistema por niveles
// ═══════════════════════════════════════════════════════════

const TILE = 32;
const GAME_NAME = 'Las 7 Cacas Maestras';
const SAVE_KEY = 'las-7-cacas-maestras-progress';
const SEQUEL_GAME_URL = '../el-pipi/index.html';

const GAME_OVER_LINES = [
  'Valentín ha caído en batalla…',
  'El Gran Cacas ríe desde lejos. Game Over.',
  'Has perdido. El trono sigue vacío.',
  'Valentín necesita más entrenamiento.',
  'El reino llora por su héroe caído.',
];
const BOSS_DEFEAT_LINES = {
  default: name => `${name} ha caído y ha soltado la llave del muro sagrado.`,
  'Gran Cacas': 'El Gran Cacas ruge una última vez… y suelta la llave. ¡Corre al altar!',
};
const LEVEL_UP_JOKES = [
  '¡Más fuerte! Las cacas tiemblan ante ti.',
  '¡Subiste de poder! El trono se acerca.',
  'Nv. up. Los jefes sudan un poco más.',
];
...
const CACAS_BEFORE_FINAL = [0, 1, 2, 3, 4, 5];

function hasSixCacas() {
  return CACAS_BEFORE_FINAL.every(id => state.collected.has(id));
}

function canAccessLevel(index) {
  if (index > state.maxUnlockedLevel) return false;
  if (index === LEVELS.length - 1) return hasSixCacas();
  return true;
}
...
function loadProgress() {
  ...
  state.maxUnlockedLevel = hasSixCacas() ? unlocked : Math.min(unlocked, LEVELS.length - 2);
}
...
function getContinueLevelIndex() {
  const data = getSaveData();
  const maxIdx = hasSixCacas() ? LEVELS.length - 1 : LEVELS.length - 2;
  if (data.session?.levelIndex != null) return Math.min(Math.max(data.session.levelIndex, 0), maxIdx);
  return Math.min(state.maxUnlockedLevel, maxIdx);
}
...
function updateMenuContinueButton() {
  const buttons = [
    ...
    document.getElementById('btn-continue-epilogue'),
  ].filter(Boolean);
  ...
  if (can) {
    ...
    const CACAS = data.collected?.length ?? state.collected.size;
    label = `▶ Continuar — Nivel ${lv.num}: ${lv.name.split(' ').slice(0, 2).join(' ')} (${CACAS}/7) <span class="key-hint">[Espacio / C]</span>`;
  } else {
...
function getSanctuaryGateY() {
  return currentLevel().caca.y + 2;
}

function isSanctuaryLocked(tx, ty) {
  if (isGateOpen() || isLevelCacaCollected()) return false;
  const c = currentLevel().caca;
...
function pickupSanctuaryKey() {
  ...
  spawnGateOpenEffect();
  showDialog('🗝️ Llave obtenida', 'El muro sagrado se ha abierto. Sube al altar y reclama el Caca Maestra.');
}
...
function isLevelCacaCollected() {
  return state.collected.has(currentLevel().caca.id);
}

function getCacaInLevel() {
  const c = currentLevel().caca;
  if (state.collected.has(c.id)) return null;
  return c;
}

function getNearestCaca() {
  const c = getCacaInLevel();
...
function getNearestExit() {
  ...
  const ty = Math.floor(state.player.y / TILE);
  if (tx === ex.x && ty === ex.y) {
    if (!isLevelCacaCollected()) return { ...ex, locked: true };
    return ex;
  }
  return null;
}
...
function collectCaca(caca) {
  if (!isGateOpen()) {
    showDialog('🔒 Reliquia sellada', 'El muro sagrado sigue cerrado. Derrota al jefe, recoge la llave y abre el muro antes de reclamar el Caca Maestra.');
    return;
  }
  state.collected.add(caca.id);
  saveProgress();
  Sound.collect();
  spawnParticles(caca.x * TILE + TILE / 2, caca.y * TILE + TILE / 2, '#ffd700', 20, true);
  state.player.xp += 50;
  checkLevelUp();
  updateHUD();
  updateInventory();
  ...
  showDialog(`${caca.emoji} ¡${caca.name} obtenida!`, caca.desc + (isFinal
    ? ' Ve al portal de victoria. ¡El trono es tuyo!'
    : ' Ve al portal morado para continuar.'), () => {});
}
...
function checkLevelUp() {
  ...
  if (p.xp >= needed) {
    ...
    Sound.heal();
    saveProgress();
    showGameBanner('⬆️ ¡Subiste de poder!', `Nv. ${p.level}. +15 HP máx, +5 de ataque. ${pick(LEVEL_UP_JOKES)}`, 180);
  }
}
...
function completeLevel() {
  ...
  document.getElementById('level-complete-caca').textContent = `${lv.caca.emoji} ${lv.caca.name} conseguida`;
...
function showEpilogue() {
  ...
  const focusId = hasContinueSave() ? 'btn-continue-epilogue'
    : (document.getElementById('btn-continue-pipi') && !document.getElementById('btn-continue-pipi').disabled ? 'btn-continue-pipi' : 'btn-sequel');
  focusOverlayButton(focusId);
}

function updateEpilogueButtons() {
  updateMenuContinueButton();
  const pipiBtn = document.getElementById('btn-continue-pipi');
  if (!pipiBtn) return;
  try {
    const pipi = JSON.parse(localStorage.getItem('pipis-miticos-progress') || '{}');
    const pipiCan = !!(pipi.session || (pipi.unlocked ?? 0) > 0 || pipi.collected?.length);
    pipiBtn.disabled = !pipiCan;
    if (pipiCan) {
      const pipis = pipi.collected?.length ?? 0;
      const lvl = (pipi.session?.levelIndex ?? pipi.unlocked ?? 0) + 1;
      pipiBtn.innerHTML = `▶ Continuar Los Pipis Míticos — Nivel ${Math.min(lvl, 7)} (${pipis}/7)`;
    } else {
      pipiBtn.innerHTML = '▶ Continuar Los Pipis Míticos <span class="key-hint">(juega primero la 2ª parte)</span>';
    }
  } catch {
    pipiBtn.disabled = true;
  }
}
...
function openSequelContinue() {
  if (!SEQUEL_GAME_URL) return;
  sessionStorage.setItem('pipis-auto-continue', '1');
  window.location.href = SEQUEL_GAME_URL;
}
...
function showGameOver() {
  ...
  if (el) el.textContent = pick(GAME_OVER_LINES);
  document.getElementById('screen-gameover').classList.add('active');
  focusOverlayButton('btn-retry');
}
...
function updateHUD() {
  ...
  document.getElementById('caca-count').textContent = `${state.collected.size} / 7`;
...
function updateInventory() {
  const list = document.getElementById('caca-list');
  list.innerHTML = '';
...
function renderLevelSelect() {
  ...
  LEVELS.forEach((lv, i) => {
    ...
    const locked = !canAccessLevel(i);
    btn.className = 'level-btn' + (locked ? ' locked' : '') + (state.collected.has(lv.caca.id) ? ' done' : '');
    btn.innerHTML = locked
      ? `🔒 ${lv.num}`
      : `${lv.num}. ${lv.name.split(' ')[0]}`;
    btn.title = i === LEVELS.length - 1 && !hasSixCacas()
      ? `${lv.name} — reúne las 6 Cacas Maestras primero`
      : lv.name;
    if (!locked) btn.onclick = () => startFromLevel(i);
    ...
    if (Math.hypot(p.x - e.px, p.y - e.py) < reach + e.radius) {
      ...
      if (e.hp <= 0) {
        ...
        if (e.boss) {
          ...
          const msg = BOSS_DEFEAT_LINES[e.name] ?? BOSS_DEFEAT_LINES.default(e.name);
          showDialog('💀 ¡Jefe derrotado!', msg);
        }
      }
    }
  }
...
function interact() {
  ...
  if (exit) {
    if (exit.locked) {
      showDialog('🔒 Portal sellado', 'Primero debes derrotar al jefe y recoger el Caca Maestra de este nivel.');
      return;
    }
    completeLevel();
    return;
...
function render() {
  ...
  const layers = [];
  const caca = getCacaInLevel();
  if (caca) {
    const near = Math.hypot(state.player.x - (caca.x * TILE + TILE / 2), state.player.y - (caca.y * TILE + TILE / 2)) < 48;
    layers.push({ y: caca.y, draw: () => Gfx.drawCaca(ctx, caca.x * TILE + TILE / 2 - cam.x, caca.y * TILE + TILE / 2 - cam.y, caca, near) });
  }
  ...
  if (tx === ex.x && ty === ex.y) {
    Gfx.drawPortalLabel(ctx, ex.x * TILE + TILE / 2 - cam.x, ex.y * TILE - cam.y - 10,
      !isLevelCacaCollected() ? '🔒 ' + ex.label : '[Espacio] ' + ex.label,
      !isLevelCacaCollected());
  }

  Gfx.drawMinimap(ctx, w, p.x, p.y, CACAS, state.collected);
...
function continueGame() {
  ...
  if (data.session) {
    ...
    const maxIdx = hasSixCacas() ? LEVELS.length - 1 : LEVELS.length - 2;
    const idx = Math.min(Math.max(s.levelIndex, 0), maxIdx);
...
function startFromLevel(index) {
  if (!canAccessLevel(index)) {
    if (index === LEVELS.length - 1 && !hasSixCacas()) {
      showDialog('🔒 Trono sellado', 'Primero debes conseguir las 6 Cacas Maestras de los niveles anteriores. Derrota a cada jefe, abre el muro sagrado y recógelas.');
    }
    return;
  }
...
document.getElementById('btn-continue-epilogue').addEventListener('click', continueGame);
document.getElementById('btn-continue-pipi').addEventListener('click', openSequelContinue);
...
document.getElementById('btn-sequel').addEventListener('click', openSequelGame);
...
if (headerTitle) headerTitle.textContent = `💩 ${GAME_NAME}`;
