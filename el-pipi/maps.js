// ═══ Mapas — Los Pipis Míticos (7 niveles únicos) ═══
...
const LEVELS = [
  {
    num: 1, id: 'level_1', name: 'Ciudad Dorada', subtitle: 'Calles de oro líquido',
    intro: 'Nivel 1: La Ciudad Dorada brilla bajo el sol. Derrota al Pipi Brillante, abre el muro sagrado y reclama el primer Pipi Mítico.',
    map: levelMap_ciudad(), cols: 20, rows: 12,
    spawn: { x: 10, y: 10 }, exit: { x: 10, y: 11, label: 'Siguiente nivel →' },
    pipi: { id: 0, name: 'Pipi Solar', emoji: '☀️💦', x: 13, y: 1,
      desc: 'Brilla como el amanecer. Calienta hasta los huesos.' },
    enemies: [
      { x: 7, y: 8, patrol: [{ dx: 1, dy: 0 }, { dx: -1, dy: 0 }], name: 'Gota Ámbar', color: '#e8b020', hp: 38 },
      { x: 11, y: 7, patrol: [{ dx: 0, dy: 1 }, { dx: 0, dy: -1 }], name: 'Gota Ámbar', color: '#d4a010', hp: 38 },
      { x: 9, y: 6, patrol: [{ dx: 1, dy: 0 }, { dx: -1, dy: 0 }], name: 'Pipi Brillante', color: '#ffd700', hp: 130, boss: true, damage: 23 },
    ],
    heals: [{ x: 6, y: 10, amount: 30 }],
    npc: { x: 4, y: 10, name: 'Sabio Pipín',
      text: '¡Valentín! Tras dominar las cacas, los Pipis Míticos despertaron. Cada reliquia está tras un muro sagrado. Derrota al jefe, recoge la llave y reclama el pipi.' },
  },
  {
    num: 2, id: 'level_2', name: 'Caverna de Ecos', subtitle: 'Sombras que gotean',
    intro: 'Nivel 2: La Caverna de Ecos responde a cada paso. Cuidado con los fosos y el Pipi Espectral.',
    map: levelMap_caverna(), cols: 18, rows: 14,
    spawn: { x: 9, y: 12 }, exit: { x: 9, y: 13, label: 'Siguiente nivel →' },
    pipi: { id: 1, name: 'Pipi Espectral', emoji: '👻💦', x: 9, y: 1,
      desc: 'Invisible al ojo… excepto cuando te moja los zapatos.' },
    enemies: [
      { x: 6, y: 7, patrol: [{ dx: 1, dy: 0 }, { dx: -1, dy: 0 }], name: 'Eco Húmedo', color: '#6688aa', hp: 42 },
      { x: 9, y: 5, patrol: [{ dx: 0, dy: 1 }, { dx: 0, dy: -1 }], name: 'Pipi Espectral', color: '#88aacc', hp: 160, boss: true, damage: 25 },
    ],
  ...
  {
    num: 7, id: 'level_7', name: 'Salón del Gran Pipi', subtitle: 'La batalla mítica',
    intro: 'Nivel 7: El Salón del Gran Pipi. Derrota al señor de los pipis y reclama el Pipi Supremo. ¡El destino dorado está en tus manos!',
    map: levelMap_salon(), cols: 18, rows: 12,
    spawn: { x: 9, y: 10 }, exit: { x: 9, y: 11, label: '🏆 Victoria' },
    pipi: { id: 6, name: 'Pipi Supremo', emoji: '👑💦', x: 9, y: 1,
      desc: 'La gota final. Quien la posea gobernará todo el reino dorado.' },
    enemies: [
      { x: 9, y: 4, patrol: [{ dx: 0, dy: 0 }, { dx: 0, dy: 0 }], name: 'Gran Pipi Mítico', color: '#d4a017', hp: 520, boss: true, radius: 22,
        bossConfig: {
          damage: [36, 54, 72],
          chase: [1.6, 2.8, 3.6],
          phaseMsgs: [
            null,
            ['💛 ¡Gran Pipi Mítico se enfurece!', 'Más rápido y lanza ondas de choque doradas. ¡No te quedes cerca!'],
            ['💀 ¡FURIA MÍTICA!', 'Proyectiles de pipi + daño brutal. ¡Esquiva todo!'],
          ],
          slamFromPhase: 1,
