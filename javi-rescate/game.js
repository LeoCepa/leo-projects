// ═══════════════════════════════════════════════════════════
//  JAVI: RESCATE DE LEO Y NARA
// ═══════════════════════════════════════════════════════════

const TILE = 32;
const GAME_NAME = 'Javi: Rescate de Leo y Nara';
const SAVE_KEY = 'javi-rescate-progress';

const GAME_OVER_LINES = [
  'Javi ha caído… Leo y Nara aún lo necesitan.',
  'El Secuestrador ríe en la oscuridad. Game Over.',
  'Has perdido. Los niños siguen esperando.',
  'Javi se levantará. Un papá nunca se rinde.',
  'Leo y Nara creen en ti. ¡Inténtalo otra vez!',
];
const BOSS_DEFEAT_LINES = {
  default: name => `${name} ha caído y ha soltado la llave del muro.`,
  'Secuestrador de Sombras': 'El Secuestrador se desvanece… ¡Corre a liberar a Leo y Nara!',
};
const LEVEL_UP_JOKES = [
  '¡Más fuerte! Cada paso acerca a la familia.',
  '¡Subiste de poder! Leo y Nara te esperan.',
  'Nv. up. Javi no se detiene por nada.',
];
...
const RELICS_BEFORE_FINAL = LEVELS.slice(0, -1).map(lv => lv.relic.id);
const TOTAL_LEVELS = LEVELS.length;
const RESCUE_LEVEL_INDEXES = [9]; // Nivel 10: rescate de Leo y Nara

function getRelicDisplayEmoji(relic, collected = true) {
  if (!collected) return '❓';
  if (relic.id === 9) return '👨‍👦👧';
  return relic.emoji;
}

function hasRelicsBeforeFinal() {
  return RELICS_BEFORE_FINAL.every(id => state.collected.has(id));
}

function canAccessLevel(index) {
  if (index > state.maxUnlockedLevel) return false;
  if (index === LEVELS.length - 1) return hasRelicsBeforeFinal();
  return true;
}
...
function loadProgress() {
  ...
  state.maxUnlockedLevel = hasRelicsBeforeFinal() ? unlocked : Math.min(unlocked, LEVELS.length - 2);
}
...
function getContinueLevelIndex() {
  const data = getSaveData();
  const maxIdx = hasRelicsBeforeFinal() ? LEVELS.length - 1 : LEVELS.length - 2;
...
function updateMenuContinueButton() {
  ...
  if (can) {
    ...
    const count = data.collected?.length ?? state.collected.size;
    label = `▶ Continuar — Nivel ${lv.num}: ${lv.name.split(' ').slice(0, 2).join(' ')} (${count}/${TOTAL_LEVELS}) <span class="key-hint">[Espacio / C]</span>`;
  } else {
...
function loadLevel(index, { resetPlayer = false, showIntro = true } = {}) {
  ...
  if (showIntro && !RESCUE_LEVEL_INDEXES.includes(index)) {
...
function getSanctuaryGateY() { return currentLevel().relic.y + 2;
}

function isSanctuaryLocked(tx, ty) {
  if (isGateOpen() || isLevelRelicCollected()) return false;
  const c = currentLevel().relic;
...
function pickupSanctuaryKey() {
  ...
  spawnGateOpenEffect();
  showDialog('🗝️ Llave obtenida', 'El muro se ha abierto. Sube y reclama el destello de esperanza.');
}
...
function isLevelRelicCollected() {
  return state.collected.has(currentLevel().relic.id);
}

function getRelicInLevel() {
  const c = currentLevel().relic;
  if (state.collected.has(c.id)) return null;
  return c;
}

function getNearestRelic() {
  const c = getRelicInLevel();
...
function getNearestExit() {
  ...
  const ty = Math.floor(state.player.y / TILE);
  if (tx === ex.x && ty === ex.y) {
    if (!isLevelRelicCollected()) return { ...ex, locked: true };
    return ex;
  }
  return null;
}
...
function handleScreenContinue() {
  ...
  switch (screenId) {
    ...
    case 'screen-epilogue':
      if (hasContinueSave()) continueGame();
      else { document.getElementById('screen-epilogue').classList.remove('active'); startGame(); }
      return true;
...
function collectRelic(relic) {
  if (!isGateOpen()) {
    showDialog('🔒 Destello sellado', 'El muro sigue cerrado. Derrota al jefe, recoge la llave y abre el muro antes de reclamar el destello.');
    return;
  }
  state.collected.add(relic.id);
  saveProgress();
  Sound.collect();
  spawnParticles(relic.x * TILE + TILE / 2, relic.y * TILE + TILE / 2, '#ffd700', 20, true);
  state.player.xp += 50;
  checkLevelUp();
  updateHUD();
  updateInventory();
  ...
  const rescueEmoji = isFinal ? '👨‍👦👧' : relic.emoji;
  showDialog(`${rescueEmoji} ¡${relic.name}!`, relic.desc + (isFinal
    ? ' Ve al portal. ¡La familia está completa!'
    : ' Ve al portal azul para continuar.'), () => {});
}
...
function completeLevel() {
  ...
  document.getElementById('level-complete-relic').textContent = `${getRelicDisplayEmoji(lv.relic)} ${lv.relic.name} conseguida`;
...
function showEpilogue() {
  ...
  focusOverlayButton(hasContinueSave() ? 'btn-continue-epilogue' : 'btn-epilogue');
}
...
function updateHUD() {
  ...
  document.getElementById('relic-count').textContent = `${state.collected.size} / ${TOTAL_LEVELS}`;
  document.getElementById('level-stage').textContent = `Nivel ${lv.num}/${TOTAL_LEVELS}`;
...
function updateInventory() {
  ...
  for (const c of RELICS) {
    ...
    if (state.collected.has(c.id)) li.textContent = `${getRelicDisplayEmoji(c)} ${c.name}`;
...
function renderLevelSelect() {
  ...
  LEVELS.forEach((lv, i) => {
    ...
    btn.className = 'level-btn' + (locked ? ' locked' : '') + (state.collected.has(lv.relic.id) ? ' done' : '');
    btn.innerHTML = locked
      ? `🔒 ${lv.num}`
      : `${lv.num}. ${lv.name.split(' ')[0]}`;
    btn.title = i === LEVELS.length - 1 && !hasRelicsBeforeFinal()
      ? `${lv.name} — reúne los ${RELICS_BEFORE_FINAL.length} destellos primero`
      : lv.name;
...
function interact() {
  ...
  if (exit) {
    if (exit.locked) {
      showDialog('🔒 Portal sellado', 'Primero debes derrotar al jefe y recoger el Destello de Esperanza de este nivel.');
      return;
    }
    completeLevel();
    return;
  ...
  if (gate) {
    if (state.keyDrop)
      ...
      showDialog('🔒 Muro sellado', 'Derrota al jefe para que suelte la llave del muro.');
    else
...
function render() {
  ...
  const layers = [];
  const relic = getRelicInLevel();
  if (relic) {
    const near = Math.hypot(state.player.x - (relic.x * TILE + TILE / 2), state.player.y - (relic.y * TILE + TILE / 2)) < 48;
    layers.push({ y: relic.y, draw: () => Gfx.drawRelic(ctx, relic.x * TILE + TILE / 2 - cam.x, relic.y * TILE + TILE / 2 - cam.y, relic, near) });
  }
  ...
  if (tx === ex.x && ty === ex.y) {
    Gfx.drawPortalLabel(ctx, ex.x * TILE + TILE / 2 - cam.x, ex.y * TILE - cam.y - 10,
      !isLevelRelicCollected() ? '🔒 ' + ex.label : '[Espacio] ' + ex.label,
      !isLevelRelicCollected());
  }

  Gfx.drawMinimap(ctx, w, p.x, p.y, RELICS, state.collected);
...
function startFromLevel(index) {
  if (!canAccessLevel(index)) {
    if (index === LEVELS.length - 1 && !hasRelicsBeforeFinal()) {
      showDialog('🔒 Corazón sellado', `Primero debes conseguir los ${RELICS_BEFORE_FINAL.length} destellos de los niveles anteriores. Derrota a cada jefe, abre el muro y recógelos.`);
    }
    return;
  }
