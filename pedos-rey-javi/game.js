// ═══════════════════════════════════════════════════════════
//  LOS PEDOS DEL REY JAVI — Tercera parte
// ═══════════════════════════════════════════════════════════

const TILE = 32;
const SAVE_KEY = 'pedos-rey-javi-progress';
const PREQUEL_GAME_URL = '../el-pipi/index.html';

const REY_JAVI_SLAM_LINES = [
  '¡Perdón, es la fabes de ayer!',
  'Eso no era un trueno… era yo.',
  '¡Silencio en la corte! …ups.',
  'Mi médico dijo que era «normal».',
  '¿Alguien trajo ambientador?',
  '¡Esto es arte real, no pedo!',
  'Valentín, ¡no te quejes del olor!',
  '¡Por el trono! …perdón.',
  'He comido lentejas de gala.',
  '¡Un pedo real para un rey real!',
  'No es miedo, es estrategia olfativa.',
  '¡Cuánta calidad en tan poco aire!',
  '¡El trono vibra! …o soy yo.',
  '¡Ni el dragón huele así!',
  '¡Era un regalo… para ti!',
  '¡Fabes con denominación de origen!',
];

const REY_JAVI_PROJECTILE_LINES = [
  '¡Proyectil aromático incoming!',
  '¡Agáchate o te pinto de verde!',
  '¡Bomba de fabes a la vista!',
  '¡Esto es guerra química, Valentín!',
  '¡Mi ejército huele a victoria!',
  '¡Disparo guiado por el viento!',
  '¡Ni la catapulta tira así!',
  '¡Fabes balística real!',
];

const REY_JAVI_TAUNT_LINES = [
  '¡Mi reino, mis reglas, mis gases!',
  '¿Eso es una espada o un pincho?',
  '¡La corona pesa, el pedo también!',
  '¡Nadie me ha vencido desde el banquete!',
  '¡Hueles a miedo… y a pipi!',
  '¡Un rey no pide perdón dos veces!',
  '¡Fabes del trono, directo a tu cara!',
];

const REY_JAVI_DEFEAT_LINES = [
  'Me rindo… pero el olor se queda. ¡Toma la llave!',
  'Has vencido al aroma supremo. La llave es tuya… *puf*',
  'Nunca pensé que alguien aguantaría mis fabes. Bien jugado.',
  'El trono es tuyo… yo me voy a ventilar la capa.',
];
...
function isReyJavi(e) {
  return !!(e?.name && e.name.toLowerCase().includes('rey javi'));
}

function spawnBubble(x, y, text, life = 90) {
  state.bubbles.push({ x, y, text, life, maxLife: life, vy: -0.5 });
}
...
function reyJaviFartLine(boss, kind = 'slam') {
  if (!isReyJavi(boss)) return;
  const now = Date.now();
  const cooldown = kind === 'proj' ? 2800 : 2200;
  if (boss.lastFartLine && now - boss.lastFartLine < cooldown) return;
  boss.lastFartLine = now;
  const lines = kind === 'proj' ? REY_JAVI_PROJECTILE_LINES : REY_JAVI_SLAM_LINES;
  spawnBubble(boss.px, boss.py - 42, pick(lines));
}
...
function loadLevel(index, { resetPlayer = false, showIntro = true } = {}) {
  ...
  if (showIntro) {
    ...
    if (lv.id === 'level_7') {
      setTimeout(() => showGameBanner('👑 Rey Javi', '«¿Valentín? Qué honor… huele a derrota. Prepárate: hoy el palacio suelta historia.»', 260), 1400);
    }
  }
...
function pickupSanctuaryKey() {
  ...
  spawnGateOpenEffect();
  showDialog('🗝️ Llave obtenida', 'El muro real se ha abierto. Sube al altar y reclama el Pedo Mítico.');
}
...
function collectPipi(pipi) {
  if (!isGateOpen()) {
    showDialog('🔒 Reliquia sellada', 'El muro real sigue cerrado. Derrota al jefe, recoge la llave y abre el muro antes de reclamar el Pedo Mítico.');
    return;
  }
  ...
  showDialog(`${pipi.emoji} ¡${pipi.name} obtenido!`, pipi.desc + (isFinal
    ? ' Ve al portal del sur para reclamar tu trono del aire.'
    ...
    if (Math.hypot(p.x - e.px, p.y - e.py) < reach + e.radius) {
      ...
      if (e.hp <= 0) {
        ...
        if (e.boss) {
          ...
          if (isReyJavi(e)) {
            showDialog('💨 ¡Rey Javi derrotado!', pick(REY_JAVI_DEFEAT_LINES));
          } else {
...
function interact() {
  ...
  if (exit) {
    if (exit.locked) {
      showDialog('🔒 Portal sellado', 'Primero debes derrotar al jefe y recoger el Pedo Mítico de este nivel.');
      return;
    }
    completeLevel();
    return;
...
function onKeyDown(e) {
  ...
  if (e.code === 'KeyC' && !e.repeat && hasContinueSave()) {
    const screen = getActiveScreenId();
    if (screen === 'screen-menu' || screen === 'screen-intro' || screen === 'screen-epilogue' || screen === 'screen-gameover') {
      e.preventDefault();
      continueGame();
    }
  }
...
function updateEnemies() {
  ...
  for (const e of state.enemies) {
    ...
    if (isChasing) {
      ...
      if (isReyJavi(e)) {
        const now = Date.now();
        if (!e.lastTaunt || now - e.lastTaunt > 9000) {
          if (Math.random() < 0.45) {
            e.lastTaunt = now;
            spawnBubble(e.px, e.py - 52, pick(REY_JAVI_TAUNT_LINES));
          }
        }
      }
    }
...
function render() {
  ...
  for (const b of state.bubbles)
    Gfx.drawBubble(ctx, b.x - cam.x, b.y - cam.y, b.text, b.life, b.maxLife);
...
updateInventory();
updateContinueButtons();
if (sessionStorage.getItem('pedos-auto-continue') === '1') {
  sessionStorage.removeItem('pedos-auto-continue');
  if (hasContinueSave()) continueGame();
