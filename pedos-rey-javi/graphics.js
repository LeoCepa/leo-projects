...
const Gfx = {
  ...
  drawPlayer(ctx, x, y, facing, walkFrame, attacking, jumpH = 0) {
    ...
    // Capa con volumen
    const cape = ctx.createLinearGradient(-12, -8, 12, 14);
    cape.addColorStop(0, '#b83828'); cape.addColorStop(1, '#6b1a12');
    ctx.fillStyle = cape;
    ctx.beginPath();
    ctx.moveTo(-10, -6); ctx.lineTo(10, -6);
    ctx.quadraticCurveTo(14 + Math.sin(this.time / 200) * 2, 4, 8, 12);
    ctx.lineTo(-8, 12);
    ctx.quadraticCurveTo(-14 - Math.sin(this.time / 200) * 2, 4, -10, -6);
    ctx.fill();
    ...
    ctx.fillRect(-5, 7 + legOff, 2, 4); ctx.fillRect(3, 7 - legOff, 2, 4);

    // Botas con brillo
    const boot = ctx.createLinearGradient(-7, 12, -1, 16);
    boot.addColorStop(0, '#3d2418'); boot.addColorStop(1, '#1a0e08');
    ctx.fillStyle = boot;
    ctx.fillRect(-7, 12 + legOff, 6, 4);
    ctx.fillRect(1, 12 - legOff, 6, 4);

    // Armadura con metal
    const armor = ctx.createLinearGradient(-9, -8, 9, 8);
    armor.addColorStop(0, '#8eb8d0'); armor.addColorStop(0.45, '#5d8aa8'); armor.addColorStop(1, '#3a5a70');
    ctx.fillStyle = armor;
    ...
    const helm = ctx.createLinearGradient(-9, -22, 9, -14);
    helm.addColorStop(0, '#d5dbe0'); helm.addColorStop(1, '#7f8c8d');
    ctx.fillStyle = helm;
    ...
    ctx.fillRect(-4, -15.5, 8, 1.5);

    // Espada
    const swordAng = attacking ? -0.8 : (facing.y < -0.3 ? -1.2 : facing.x > 0.3 ? 0.3 : -0.5);
    ctx.save();
    ctx.rotate(swordAng);
    ctx.fillStyle = '#ecf0f1'; ctx.fillRect(10, -2, 16, 4);
    ctx.fillStyle = '#bdc3c7'; ctx.fillRect(10, -1, 14, 2);
    ctx.fillStyle = '#8b6914'; ctx.fillRect(6, -5, 6, 10);
    ctx.fillStyle = '#ffd700'; ctx.fillRect(7, -2, 4, 4);
    if (attacking) {
      ctx.strokeStyle = 'rgba(255,255,200,0.7)'; ctx.lineWidth = 2;
      ctx.beginPath(); ctx.arc(14, 0, 12, -0.5, 0.8); ctx.stroke();
    }
    ctx.restore();
  ...
  drawEnemy(ctx, e, x, y) {
    ...
    const isGranCacas = (e.name || '').toLowerCase().includes('gran cacas');
    const isGranPipi = (e.name || '').toLowerCase().includes('gran pipi');
    const isReyJavi = (e.name || '').toLowerCase().includes('rey javi');

    if (isReyJavi) this._drawReyJavi(ctx, r, e.phase, e.facing);
    else if (isGranPipi) this._drawGranPipi(ctx, r, e.phase);
    else if (isGranCacas) this._drawGranCacas(ctx, r, e.phase);
  ...
  _drawGranPipi(ctx, r, phase) {
    const scale = 1.1 + phase * 0.15;
    ctx.scale(scale, scale);
    const body = ctx.createRadialGradient(-r * 0.2, -r * 0.2, 2, 0, 2, r);
    body.addColorStop(0, phase >= 2 ? '#fff8a0' : '#ffe566');
    body.addColorStop(0.6, phase >= 1 ? '#d4a017' : '#e8c040');
    body.addColorStop(1, '#9a7010');
    ctx.fillStyle = body;
    ctx.beginPath(); ctx.moveTo(0, -r * 0.9); ctx.quadraticCurveTo(r * 0.8, -r * 0.2, 0, r); ctx.quadraticCurveTo(-r * 0.8, -r * 0.2, 0, -r * 0.9); ctx.fill();
    ctx.fillStyle = '#4a90d9';
    ctx.beginPath(); ctx.ellipse(0, r * 0.55, r * 0.35, r * 0.2, 0, 0, Math.PI * 2); ctx.fill();
    ctx.fillStyle = '#ffd700';
    ctx.fillRect(-10, -r - 4, 20, 5);
    ctx.fillRect(-7, -r - 10, 4, 7); ctx.fillRect(-1, -r - 12, 4, 9); ctx.fillRect(5, -r - 10, 4, 7);
    ctx.fillStyle = phase >= 1 ? '#2266cc' : '#4488dd';
    ctx.beginPath(); ctx.arc(-6, -r * 0.15, 4, 0, Math.PI * 2); ctx.arc(6, -r * 0.15, 4, 0, Math.PI * 2); ctx.fill();
    if (phase >= 2) {
      ctx.fillStyle = `rgba(255,230,80,${0.55 + Math.sin(this.time / 90) * 0.25})`;
      ctx.beginPath(); ctx.moveTo(-r, 2); ctx.lineTo(-r - 14, 8); ctx.lineTo(-r + 4, 6); ctx.fill();
      ctx.beginPath(); ctx.moveTo(r, 2); ctx.lineTo(r + 14, 8); ctx.lineTo(r - 4, 6); ctx.fill();
    }
  },
  ...
  drawSlamWave(ctx, x, y, radius, life) {
    ...
    ctx.strokeStyle = `rgba(120,200,60,${alpha})`;
    ctx.lineWidth = 4;
    ctx.beginPath(); ctx.arc(x, y, radius, 0, Math.PI * 2); ctx.stroke();
    ctx.strokeStyle = `rgba(180,255,100,${alpha * 0.5})`;
    ctx.lineWidth = 2;
  ...
  _enemyKind(e) {
    ...
    if (n.includes('pipi') || n.includes('gota') || n.includes('corrosivo') || n.includes('prisma')) return 'slime';
    if (n.includes('drag') || n.includes('salamandra') || n.includes('ígneo') || n.includes('gran cacas')) return 'dragon';
  ...
  drawCaca(ctx, x, y, caca, near) {
    ...
    // Aura volumétrica
    const pulse = 0.35 + Math.sin(t / 250) * 0.2;
    const grad = ctx.createRadialGradient(x, cy, 2, x, cy, 32);
    grad.addColorStop(0, hue + 'cc');
    grad.addColorStop(0.4, hue + '55');
    grad.addColorStop(1, 'transparent');
    ctx.fillStyle = grad;
    ctx.beginPath(); ctx.arc(x, cy, 32, 0, Math.PI * 2); ctx.fill();

    for (let i = 0; i < 5; i++) {
      const ang = t / 600 + i * Math.PI * 2 / 5;
      const dist = 16 + Math.sin(t / 400 + i) * 3;
      ctx.fillStyle = hue;
      ctx.globalAlpha = 0.55 + Math.sin(t / 300 + i) * 0.2;
      ctx.beginPath();
      ctx.arc(x + Math.cos(ang) * dist, cy + Math.sin(ang) * dist * 0.55, 2.5, 0, Math.PI * 2);
      ctx.fill();
    }
    ctx.globalAlpha = 1;
    ...
    if (near) {
      ...
      ctx.fillStyle = '#ffd700';
      ctx.strokeStyle = '#000'; ctx.lineWidth = 3;
      ctx.strokeText('[Espacio]', x, cy - 30);
      ctx.fillText('[Espacio]', x, cy - 30);
    }
  },

  drawPipi(ctx, x, y, pipi, near) {
    const t = this.time;
    const bob = Math.sin(t / 350 + pipi.id * 1.5) * 4;
    const cy = y + bob;
    const hues = ['#ffe566','#88ccff','#5eb0e8','#66ddff','#aadd00','#cc88ff','#ffd740'];
    const hue = hues[pipi.id] || '#ffe566';
    ...
    ctx.lineTo(x + 12, y + 8); ctx.lineTo(x - 12, y + 8); ctx.closePath(); ctx.fill();
    const ped = ctx.createLinearGradient(x - 12, y, x + 12, y + 8);
    ped.addColorStop(0, '#a0b8d8'); ped.addColorStop(1, '#4a6080');
    ctx.fillStyle = ped;
    ctx.fillRect(x - 12, y + 2, 24, 8);
    const gold = ctx.createLinearGradient(x - 8, y, x + 8, y + 4);
    gold.addColorStop(0, '#fff8c0'); gold.addColorStop(1, '#d4a017');
    ctx.fillStyle = gold;
    ctx.fillRect(x - 9, y, 18, 4);
    ...
    const drop = ctx.createLinearGradient(x, cy - 14, x, cy + 10);
    drop.addColorStop(0, '#fff8a0');
    drop.addColorStop(0.5, hue);
    drop.addColorStop(1, '#4a90d9');
    ctx.fillStyle = drop;
    ctx.beginPath();
    ctx.moveTo(x, cy - 12);
    ctx.quadraticCurveTo(x + 10, cy + 2, x, cy + 10);
    ctx.quadraticCurveTo(x - 10, cy + 2, x, cy - 12);
    ctx.fill();
    ctx.fillStyle = 'rgba(255,255,255,0.45)';
    ctx.beginPath(); ctx.ellipse(x - 3, cy - 4, 3, 5, -0.3, 0, Math.PI * 2); ctx.fill();
